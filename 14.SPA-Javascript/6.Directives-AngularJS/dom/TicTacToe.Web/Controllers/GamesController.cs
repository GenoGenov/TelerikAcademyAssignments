...
using TicTacToe.Web.Infrastructure;
    ...
    public class GamesController : BaseApiController
    {
        ...
        public IHttpActionResult Play(PlayRequestDataModel request)
        {
            ...
            this.data.SaveChanges();
            var currentPlayer = this.data.Users.All().First(x => x.Id == currentUserId);
            var otherPlayerId = game.FirstPlayerId == currentUserId ? game.SecondPlayerId : game.FirstPlayerId;
            var otherPlayer = this.data.Users.All().First(x => x.Id == otherPlayerId);
            ...
            switch (gameResult)
            {
                ...
                case GameResult.WonByX:
                    var winner = this.data.Users.All().First(x => x.Id == game.FirstPlayerId);
                    var loser = this.data.Users.All().First(x => x.Id == game.SecondPlayerId);
                    winner.Wins += 1;
                    loser.Losses += 1;
                ...
                case GameResult.WonByO:
                    var winnerPlayer = this.data.Users.All().First(x => x.Id == game.SecondPlayerId);
                    var loserPlayer = this.data.Users.All().First(x => x.Id == game.FirstPlayerId);
                    winnerPlayer.Wins += 1;
        ...
        {
            var games = this.data.Games.All().ToList();
